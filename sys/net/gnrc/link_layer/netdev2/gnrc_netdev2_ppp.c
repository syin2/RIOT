...
static gnrc_pktsnip_t *_recv(gnrc_netdev2_t *gnrc_netdev2)
{
    ...
    if (bytes_expected) {
        ...
        /* mark hdlc header */
        gnrc_pktsnip_t *hdlc_hdr = gnrc_pktbuf_mark(pkt, sizeof(hdlc_hdr_t), GNRC_NETTYPE_HDLC);
        if (!hdlc_hdr) {
		...
		if (hdlc_hdr_get_address(&hdr) != HDLCTYPE_PPP){
		...
		/* Set packet type */
		switch(hdlc_hdr_get_protocol(&hdr))
		{
			case PPPTYPE_LCP:
				pkt->type = GNRC_NETTYPE_PPP_LCP;
				break;
			case PPPTYPE_NCP_IPV4:
				pkt->type = GNRC_NETTYPE_PPP_IPCP;
				break;
			case PPPTYPE_IPV4:
...
static int _send(gnrc_netdev2_t *gnrc_netdev2, gnrc_pktsnip_t *pkt)
{
    ...
    if (pkt->type != GNRC_NETTYPE_PPP_LCP || pkt->type != GNRC_NETTYPE_PPP_IPCP || pkt->type != GNRC_NETTYPE_IPV4) {
