...
static inline void _xor128(uint8_t *x, uint8_t *y)
{
    uint8_t i;
    for (i = 0; i < 16; i++) {
        y[i] = x[i] ^ y[i];
...
static inline void _leftshift(uint8_t *x, uint8_t *y)
{
    uint8_t i;
    for (i = 0; i < 15; i++)
    {
...
int cmac_init(cmac_context_t *ctx, const uint8_t *key, uint8_t keySize)
{
    ...
    ctx->M_n = 0;
    return cipher_init(&(ctx->aes_ctx), CIPHER_AES_128, key, keySize);
}
